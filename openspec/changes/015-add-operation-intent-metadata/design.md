...
- Риск: внедрение станет breaking change для текущего contract inventory.
  - Смягчение: implement phase должна включать явную миграцию примеров, templates и затронутых контрактов в репозитории в том же change set.
...
4. Добавить examples для `GET`-read, `POST`-read, `POST`-write.
5. Добавить contract/integration tests для explicit metadata и migration failures.

## Open Questions
- Должен ли future release вводить stricter mode, где новые контракты обязаны явно задавать `intent` и `access_pattern`?
