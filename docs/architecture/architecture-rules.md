...
| AR-006 | Запись generated/scaffold допускается только внутри `project_dir` с единым path-boundary policy; `generated_dir` и `scaffold_dir` не могут совпадать или быть вложенными друг в друга | `test_write_boundary_policy.py` | Mandatory / test-backed |
